...
This script includes a targetting ratio that varies with malaria prevalence
...
# Parameter estimates for recalculating tau come from Chitnis et al, 2019, Malaria Journal
alpha = np.array([0.23, -1.40, 2.87])
scaling_factor = np.array([0.191,0.419,0]) # factor needed to scale the targetting ratio estimated from Chitnis et al to the targetting ratio in the RADZEC study data for the index household 
# Calculation can be found in 'targetting ratio calc.py'
...
#%%
# --------- Function for updating tau for the new prevalence, -----------------
# ------------ using Chitnis et al, 2019, and then scaling --------------------

@njit
def calc_tau(nu, scaling_factor, N, prev):
    tau_estimate = np.exp((-1*alpha[0]*np.log(prev)+(alpha[1]/nu)-(alpha[2]/nu)*np.log(prev))*(N-nu)/(N))
...
@njit
def stoc_eqs(INPUT, lop, cumsum, beta, treatment_seeking, followup_prop, rdt_sensitivity, 
             neighbours, theta_in, theta_out): 
    Rate = np.zeros((6*n))
    ...
    if lop%nDays==0: # on every nDays, RCD is implemented
        iota = I*treatment_seeking*followup_prop # total number of cases followed up per day
        nu = nu_HH+neighbours-1
        prev = I/N
        tau = calc_tau(nu=nu, scaling_factor=scaling_factor, N=N, prev=prev) # update tau with the current prevalence
        phi = tau*(nu+1)*rdt_sensitivity*iota
...
def Stoch_Iteration(INPUT, cumsum):
    ...
    for lop in Time:
        if lop < 365*init_sim: # if time is within the initial time needed to reach equilibrium, run without interventions
            res,cs = stoc_eqs(INPUT, lop, cumsum, beta, treatment_seeking, followup_prop, 
                              rdt_sensitivity, neighbours=0, theta_in=theta, theta_out=theta)
            ...
            res,cs = stoc_eqs(INPUT, lop, cumsum, beta=beta_new, treatment_seeking=treatment_seeking_new, 
                              followup_prop=followup_new, rdt_sensitivity=rdt_sensitivity_new,
                              neighbours=neighbours_new, theta_in=theta_in, theta_out=theta_out)
            for ss in Time_year:
                if lop==ss:
                    S[i,:] = INPUT[0,:]
                    P[i,:] = INPUT[1,:]
